...
try: icono_pestana = Image.open(ICONO_FILE)
except: icono_pestana = "🖥️"

st.set_page_config(page_title=APP_NAME, page_icon=icono_pestana, layout="wide", initial_sidebar_state="expanded")
...
st.markdown("""
<style>
:root { --verde: #0b6e3c; --verde-hover: #158f4a; }
header[data-testid="stHeader"] { border-bottom: 4px solid var(--verde); }
header::after { display: none !important; }
section[data-testid="stSidebar"] { background-color: var(--verde) !important; padding-top: 1rem; }
section[data-testid="stSidebar"] > div,
section[data-testid="stSidebar"] [data-testid="stSidebarContent"] { background-color: var(--verde) !important; }
...
section[data-testid="stSidebar"] h2, section[data-testid="stSidebar"] h3 { color: #ffffff !important; font-size: 15px; }
section[data-testid="stSidebar"] .stButton > button, section[data-testid="stSidebar"] button {
    ...
    border: 1px solid rgba(255,255,255,0.5) !important; border-radius: 6px !important;
    font-size: 13px !important; width: 100% !important; box-shadow: none !important; }
section[data-testid="stSidebar"] .stButton > button:hover,
section[data-testid="stSidebar"] button:hover { background-color: rgba(255,255,255,0.3) !important; }
...
section[data-testid="stSidebar"] [data-testid="stSidebarNavLink"]:hover { background-color: rgba(255,255,255,0.2) !important; }
...
.main { padding: 2rem; }
.main .stButton > button { background-color: var(--verde); color: #ffffff; border-radius: 6px; border: none; padding: 0.5rem 1rem; font-weight: 600; width: 100%; }
.main .stButton > button:hover { background-color: var(--verde-hover); }
input, textarea { border-radius: 6px !important; }
footer { visibility: hidden; }
.login-box { max-width: 420px; margin: 3rem auto; padding: 2.5rem 2rem; background: #ffffff; border-radius: 12px; box-shadow: 0 4px 24px rgba(11,110,60,0.13); border-top: 5px solid #0b6e3c; }
.revision-card { background: #f8fffe; border: 1px solid #c8e6d4; border-left: 4px solid #0b6e3c; border-radius: 8px; padding: 0.8rem 1rem; margin-bottom: 0.5rem; }
...
def mostrar_login():
    ...
    try:
        if HEADER_MAIN_FILE.exists():
            ...
            with col_c: st.image(str(HEADER_MAIN_FILE), use_container_width=True)
    except: pass
    _, col_c, _ = st.columns([1, 1.4, 1])
    with col_c:
        st.markdown('<div class="login-box">', unsafe_allow_html=True)
        st.markdown(f"<h3 style='text-align:center;color:#0b6e3c;'>🔐 Acceso al Sistema</h3>", unsafe_allow_html=True)
        st.markdown(f"<p style='text-align:center;color:#666;font-size:1rem;font-weight:700;'>{APP_NAME}</p>", unsafe_allow_html=True)
...
def _banner():
    try:
        if HEADER_MAIN_FILE.exists(): st.image(str(HEADER_MAIN_FILE), width='stretch')
    except: pass
...
def listar_revisiones():
    return sorted([r for r in CARPETA_REVISIONES.iterdir() if r.is_dir()],
...
def crear_revision(nombre, archivos):
    ...
    for nom_arch, contenido in archivos:
        ...
        if nom_arch.lower().endswith(".xlsx"): xlsx_bytes = contenido
    # Auto-importar XLSX si lo hay
    if xlsx_bytes:
        conn = get_db()
        try:
            _importar_xlsx_a_bd(conn, xlsx_bytes, "revision", limpiar=True)
...
def eliminar_revision(carpeta):
    ...
    # Limpiar BD al borrar la revisión
    conn = get_db()
    try:
...
def _group(code, name=""):
    ...
    if any(k in nm for k in ("adaptador","conversor","convertidor","hub usb","dock","docking")):
        return "Adaptadores"
    if any(k in nm for k in ("cable","latigillo","latiguillo","hdmi","vga","displayport")):
        return "Cables"
    if any(k in nm for k in ("cargador","fuente de alimentacion","fuente de alimentación",
                              "bateria","batería","sai","ups","regleta","alargador")):
        return "Energía"
    if any(k in nm for k in ("raton","ratón","mouse","teclado","keyboard",
                              "auricular","auriculares","headset","cascos",
                              "webcam","camara","cámara","microfono","micrófono",
                              "monitor","pantalla","display","impresora",
                              "escaner","escáner","altavoz","altavoces")):
        return "Periféricos"
    if any(k in nm for k in ("switch","router","access point","punto de acceso",
                              "wifi","ethernet","patch panel","rack")):
        return "Switch"
    # ── fallback por código ──────────────────────────────────────────
    cat = _classify(code)
...
def _load_base(conn):
    ...
    df_profs = _sort_nat(df_profs, 'code')
...
def _importar_xlsx_a_bd(conn, xlsx_bytes, fuente="import", limpiar=False):
    ...
    if sn:
        ...
        for _, row in df.iterrows():
            ...
            ini = _ff(row.get("existencias iniciales", 0))
            ent = _ff(row.get("entradas", 0))
            sal = _ff(row.get("salidas",  0))
            try:
                conn.execute(
                    "INSERT OR REPLACE INTO articles (code,name,type,initial_stock,entries,exits,current_stock) VALUES (?,?,?,?,?,?,?)",
                    (code, row.get("nombre del articulo"), _classify(code), ini, ent, sal, ini+ent-sal))
                results["Artículos"] += 1
            except: pass

    # Profesores (hoja "profesores" o "clientes")
    ...
    if sn:
        ...
        for _, row in df.iterrows():
            ...
            try:
                conn.execute(
                    "INSERT OR REPLACE INTO clients (code,fiscal_name,name,surname,nif,phone,email,address,city,province,zip_code) VALUES (?,?,?,?,?,?,?,?,?,?,?)",
    ...
    if sn:
        ...
        for _, row in df.iterrows():
            ...
            try:
                conn.execute(
                    "INSERT OR REPLACE INTO providers (code,fiscal_name,name,surname,nif,phone,email,address,city,province,zip_code) VALUES (?,?,?,?,?,?,?,?,?,?,?)",
                    (code, row.get("nombre fiscal"), row.get("nombre"), row.get("apellidos"),
                     row.get("nif/cif"), row.get("telefono"), row.get("e-mail"),
                     row.get("direccion"), row.get("poblacion"), row.get("provincia"),
                     _sc(row.get("codigo postal", ""))))
    ...
    if sn:
        ...
        for _, row in df.iterrows():
            ...
            try:
                conn.execute(
                    "INSERT OR IGNORE INTO stock_entries (entry_code,date,provider_code,provider_name,article_code,article_name,quantity,last_modified_by) VALUES (?,?,?,?,?,?,?,?)",
                    (code, _fd(row.get("fecha")), _sc(row.get("proveedor (codigo)", "")),
                     row.get("proveedor (nombre)"), _sc(row.get("articulo (codigo)", "")),
                     row.get("articulo (nombre)"), _ff(row.get("cantidad", 0)), fuente))
                results["Entradas"] += 1
            except: pass
    ...
    if sn:
        ...
        for _, row in df.iterrows():
            ...
            try:
                conn.execute(
                    "INSERT OR IGNORE INTO stock_exits (exit_code,date,client_code,client_name,article_code,article_name,quantity,price,delivered_to,last_modified_by) VALUES (?,?,?,?,?,?,?,?,?,?)",
                    (code, _fd(row.get("fecha")), _sc(row.get("profesor (codigo)", "")),
                     row.get("profesor (nombre)"), _sc(row.get("articulo (codigo)", "")),
                     row.get("articulo (nombre)"), _ff(row.get("cantidad", 0)),
                     row.get("precio"), row.get("entregado a"), fuente))
                results["Salidas"] += 1
            except: pass
...
def pagina_existencias():
    ...
    if f_texto.strip():
        ...
        df_view = df_view[df_view['code'].str.lower().str.contains(q, na=False) |
    ...
    if not df_view.empty:
        ...
        try:
            ...
            df_c.columns = ['Categoría','Unidades']; df_c = df_c[df_c['Unidades'] > 0]
            if not df_c.empty:
    ...
    with cb3:
        if not df_view.empty:
            ...
            st.download_button("📥 Exportar Excel", data=_to_excel(df_view[cols_ex]),
                               file_name="existencias.xlsx", mime="application/vnd.ms-excel", key="exp_art")
    conn.close()
...
def pagina_entradas():
    ...
    conn = get_db(); df_arts, df_provs, _ = _load_base(conn)
    current_user        = st.session_state.usuario_actual or "usuario"
    ...
    ed_ent = st.data_editor(dv, num_rows="dynamic", key="ed_ent", column_config={
        "Sel": st.column_config.CheckboxColumn("✅", width="small"),
        "id": {"hidden": True}, "provider_code": {"hidden": True},
        "provider_name": {"hidden": True}, "article_code": {"hidden": True}, "article_name": {"hidden": True},
...
# ============================================================================
# PÁGINA 4 — SALIDAS  (clientes → profesores)
# ============================================================================
def pagina_salidas():
    _banner(); st.title("📤 Salidas")
    conn = get_db(); df_arts, _, df_profs = _load_base(conn)
    current_user       = st.session_state.usuario_actual or "usuario"
    list_articles_full = [x for x in df_arts['display_full']  if x]
    list_profs_full    = [x for x in df_profs['display_full'] if x]
    list_responsibles  = [x for x in df_profs.apply(
    ...
    map_ext = {"exit_code":"Cód.","date":"Fecha","client_name":"Profesor",
...
# ============================================================================
# PÁGINA 5 — CONFIGURACIÓN (Revisiones + Gestión de Usuarios)
# ============================================================================
def pagina_configuracion():
    ...
    with tab_rev:
        ...
        st.markdown("Sube archivos **PDF, CSV o XLSX**. Los XLSX se importan automáticamente en Existencias, Entradas y Salidas.")

        archivos_subidos = st.file_uploader(
            "📎 Arrastra aquí tus archivos (PDF, CSV, XLSX)",
            type=["pdf", "csv", "xlsx"], accept_multiple_files=True, key="rev_uploader"
        )
        ...
        with col_btn:
            ...
            if st.button("💾 Guardar revisión", key="btn_guardar_rev", use_container_width=True):
                if not archivos_subidos:
                    ...
                    st.success(f"✅ Revisión **{nombre_rev}** guardada. Los datos se han importado automáticamente.")
                    st.rerun()
            ...
            for carpeta in revisiones:
                ...
                with c4:
                    if st.button("🗑️ Eliminar", key=f"btn_del_{carpeta.name}"):
                        ...
                        st.success("🗑️ Revisión y datos eliminados."); st.rerun()

                with st.expander("➕ Añadir archivos a esta revisión", expanded=False):
                    extra = st.file_uploader("Arrastra PDF, CSV o XLSX",
                                             type=["pdf","csv","xlsx"],
                    ...
                    if st.button("Añadir a revisión", key=f"btn_add_{carpeta.name}"):
                        if extra:
                            ...
                            for f in extra:
                                ...
                                if f.name.lower().endswith(".xlsx"): xlsx_extra = contenido
                            if xlsx_extra:
                                conn = get_db()
                                try:
                                    _importar_xlsx_a_bd(conn, xlsx_extra, "revision", limpiar=True)
                                    conn.execute("INSERT OR IGNORE INTO demo_loaded (id) VALUES (1)")
                                    conn.commit(); _sync_stock(conn)
...
def main():
    ...
    with st.sidebar:
        ...
        archivos_sidebar = st.file_uploader(
            "PDF, CSV o XLSX", type=["pdf","csv","xlsx"],
    ...
    pg = st.navigation({
        APP_NAME:         [p_existencias, p_productos, p_entradas, p_salidas],
