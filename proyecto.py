...
except: icono_pestana="📂"

st.set_page_config(page_title="RPT - Gestor de Puestos",page_icon=icono_pestana,layout="wide",initial_sidebar_state="expanded")
...
section[data-testid="stSidebar"] [data-testid="stSidebarContent"]{background-color:var(--verde-junta) !important;}
...
section[data-testid="stSidebar"] .stButton>button:hover,section[data-testid="stSidebar"] button:hover{background-color:rgba(255,255,255,0.3) !important;}
...
section[data-testid="stSidebar"] [data-testid="stSidebarNavLink"][aria-current="page"]{background-color:rgba(255,255,255,0.25) !important;border-left:3px solid #ffffff !important;font-weight:700 !important;}
section[data-testid="stSidebar"] [data-testid="stSidebarNavLink"] span,
section[data-testid="stSidebar"] [data-testid="stSidebarNavLink"] p{color:#ffffff !important;}
section[data-testid="stSidebar"] nav>div>p{color:rgba(255,255,255,0.6) !important;font-size:11px !important;text-transform:uppercase;letter-spacing:0.08em;font-weight:600 !important;}
.main{padding:2rem;}
.main .stButton>button{background-color:var(--verde-junta);color:#ffffff;border-radius:6px;border:none;padding:0.5rem 1rem;font-weight:600;width:100%;}
...
.import-banner{background:#e8f5e9;border:1px solid #a5d6a7;border-radius:8px;padding:0.6rem 1rem;margin-top:0.4rem;font-size:0.85rem;color:#2e7d32;}
</style>
...
def eliminar_revision(carpeta): shutil.rmtree(carpeta)
...
def _group(code,name=""):
    cat=_classify(code); nm=str(name).lower()
...
def _load_base(conn):
    ...
    for df in [df_arts,df_provs,df_clis]: df['code']=df['code'].apply(_sc)
    df_arts=_sort_nat(df_arts,'code'); df_provs=_sort_nat(df_provs,'code'); df_clis=_sort_nat(df_clis,'code')
...
# ============================================================================
# ★ FUNCIÓN CENTRAL DE IMPORTACIÓN XLSX ★
# Recibe los bytes del xlsx y los vuelca en la BD.
# fuente = etiqueta para last_modified_by ("demo", "import", etc.)
# limpiar = True borra todas las tablas antes de importar
# ============================================================================
def _importar_xlsx_a_bd(conn, xlsx_bytes, fuente="import", limpiar=False):
    ...
    results={"Artículos":0,"Clientes":0,"Proveedores":0,"Entradas":0,"Salidas":0}
    ...
    for sheet_key,table,rkey in [("cliente","clients","Clientes"),("proveedor","providers","Proveedores")]:
        ...
        if sn:
            ...
            for _,row in df.iterrows():
                ...
                try:
                    conn.execute(
                        f"INSERT OR REPLACE INTO {table} (code,fiscal_name,name,surname,nif,phone,email,address,city,province,zip_code) VALUES (?,?,?,?,?,?,?,?,?,?,?)",
                        (code,row.get("nombre fiscal"),row.get("nombre"),row.get("apellidos"),row.get("nif/cif"),
                         row.get("telefono"),row.get("e-mail"),row.get("direccion"),row.get("poblacion"),
                         row.get("provincia"),_sc(row.get("codigo postal",""))))
                    results[rkey]+=1
                except: pass
        ...
        for _,row in df.iterrows():
            ...
            try:
                conn.execute(
                    ...
                    (code,_fd(row.get("fecha")),_sc(row.get("cliente (codigo)","")),
                     row.get("cliente (nombre)"),_sc(row.get("articulo (codigo)","")),
...
def pagina_configuracion():
    ...
    with tab_rev:
        ...
        st.markdown("Sube archivos **PDF, CSV o XLSX** y guárdalos como una revisión. Los XLSX con el formato del almacén se pueden importar directamente a la base de datos.")
        ...
        if not revisiones:
            ...
            for carpeta in revisiones:
                meta=leer_meta(carpeta); archivos=archivos_de_revision(carpeta); n_archs=len(archivos)
                xlsx_en_rev=[p for p in archivos if p.suffix.lower()==".xlsx"]
                ...
                # Botón de importar XLSX si los hay
                if xlsx_en_rev:
                    st.markdown(f'<div class="import-banner">📊 Esta revisión contiene {len(xlsx_en_rev)} archivo(s) XLSX que pueden importarse al inventario.</div>',unsafe_allow_html=True)
                    for xlsx_path in xlsx_en_rev:
                        col_xi,col_xb=st.columns([3,1])
                        with col_xi: st.caption(f"📊 {xlsx_path.name}")
                        with col_xb:
                            if st.button(f"📥 Importar a BD",key=f"imp_{carpeta.name}_{xlsx_path.name}",type="primary"):
                                with st.spinner(f"Importando {xlsx_path.name}..."):
                                    conn=get_db()
                                    try:
                                        results=_importar_xlsx_a_bd(conn,xlsx_path.read_bytes(),"revision",limpiar=True)
                                        # marcar demo_loaded para que no se sobreescriba
                                        conn.execute("INSERT OR IGNORE INTO demo_loaded (id) VALUES (1)")
                                        conn.commit(); _sync_stock(conn); _clear_cache()
                                        st.success(f"✅ Importado correctamente: {results}")
    ...
    with tab_imp:
        ...
        st.warning("⚠️ Esta acción **reemplaza todos los datos actuales** de la base de datos.")
        ...
        if u and st.button("Procesar Archivo",key="btn_import"):
            with st.spinner("Procesando..."):
                conn=get_db()
                try:
                    results=_importar_xlsx_a_bd(conn,u.read(),"import",limpiar=True)
                    conn.execute("INSERT OR IGNORE INTO demo_loaded (id) VALUES (1)")
                    conn.commit(); _sync_stock(conn); _clear_cache()
